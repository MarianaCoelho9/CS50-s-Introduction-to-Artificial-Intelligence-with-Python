...
class CrosswordCreator():
        ...
        if arcs is None:
            ...
            for variable1 in self.crossword.variables:
                for variable2 in self.crossword.variables:
                    if variable1!=variable2 and (not (variable1,variable2) in queue) and (not (variable2,variable1) in queue):
    ...
    def assignment_complete(self, assignment):
        ...
        """
        if assignment.keys() == self.crossword.variables and all(assignment.values()):
            return True
        else:
            return False
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for variable in self.crossword.variables:
            if not (variable in list(assignment.keys())):
                if len(self.domains[variable])==n_domain:
                    if  len(self.crossword.neighbors(variable))>n_neighbors:
                        n_domain=len(self.domains[variable])
                        n_neighbors=len(self.crossword.neighbors(variable))
                        unassigned_variable=variable
                elif len(self.domains[variable])<n_domain:
                    n_domain=len(self.domains[variable])
                    n_neighbors=len(self.crossword.neighbors(variable))
                    unassigned_variable=variable
